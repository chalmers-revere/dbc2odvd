...
```
docker run --rm -ti --net=host -v /file/to/a/file.dbc:/file.dbc chalmersrevere/dbc2odvd:v0.0.6 cantools monitor /file.dbc
```
...
```
docker run --rm -ti -v $PWD:/in -w /in chalmersrevere/dbc2odvd:v0.0.6 generateHeaderOnly.sh myFile.dbc myFile.odvd
```
...
```
docker run --rm -ti -v $PWD:/in -w /in chalmersrevere/dbc2odvd:v0.0.6 generateMappingCodeSnippet.awk myFile.dbc.map
```
